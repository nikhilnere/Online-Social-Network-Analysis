...
#
# I have clustered the resulting graph into communities, and recommend friends for Bill Gates.
#
# 
        ...
        for node in neighbours:
            ...
            if visited[node] == 0:
                ...
                if(node2distances[current]+1 > max_depth):
                    ...
                    node2num_paths[node] = node2num_paths[current]
                    possparents = graph.neighbors(node)
                    parents = []
                    parents.append (current)
                    for possParent in possparents:
                        if possParent!=current:
                            if node2distances.__contains__(possParent):
                                if node2distances[possParent] == node2distances[current]:
                                    parents.append(possParent)
                                    paths = node2num_paths[node] + 1
                                    node2num_paths[node] = paths

                    node2parents[node] = list(parents)
                    q.append(node)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for node in node2distances:
        ...
        if node2distances[node] in leveldict:
            nodes = leveldict[node2distances[node]]
            nodes.append(node)
            leveldict[node2distances[node]] = nodes
        else:
            nodes = []
            nodes.append(node)
            leveldict[node2distances[node]] = nodes

    levels = sorted(leveldict.keys() , reverse=True)

    for level in levels:
        if (level!= 0):
            nodesatlevel = leveldict[level]
            for node in nodesatlevel:
                parents = node2parents[node]
                for parent in parents:
                    wt = float(nodecredits[node] / len(parents))
                    edgetuple = (node, parent)
                    edgetuple = sorted(edgetuple)
                    result[(edgetuple[0], edgetuple[1])] = wt
                    nodecredits[parent] = float(nodecredits[parent]) + wt
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in nodes:
        ...
        for key in result:
            if (approxbetweenness.__contains__(key)):
                approxbetweenness[key] = approxbetweenness[key] + result[key]
            else:
                approxbetweenness[key] = result[key]

    for key in approxbetweenness:
        approxbetweenness[key] = (approxbetweenness[key] / 2)
...
def partition_girvan_newman(graph, max_depth):
    ...
    if tempgraph.order()==1:
        return [tempgraph.nodes()]



    approxbetweenness = approximate_betweenness(tempgraph, max_depth)
    #sortedbetweenness = sorted (approxbetweenness.items(), key=lambda x:(-x[1],x[0]))[:4]
    sortedbetweenness = sorted(approxbetweenness.items(), key=lambda x: (-x[1], x[0]))

    components = [c for c in nx.connected_component_subgraphs(tempgraph)]

    edgeno = 0

    while len(components) == 1:
        edge = sortedbetweenness[edgeno][0]
        #print ('edge to remove : ' + str(edge))
        edgeno = edgeno + 1
...
def norm_cut(S, T, graph):
    ...
    return float(cutst/vols) + float(cutst/volt)
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    tempgraph = graph.copy()
    edgestoremove = sorted(tempgraph.edges([test_node]), key=lambda x:x[1])
    i = 0
...
def jaccard(graph, node, k):
    ...
    for n in othernodes:
        if not graph.has_edge (*(node, n)):
            ...
            jaccardcoeff.append(((node, n), 1. * len(neighbors & neighbors2) / len(neighbors | neighbors2)))

    return sorted(jaccardcoeff, key=lambda x: (-x[1],x[0][1]))[:k]
    pass
...
def path_score(graph, root, k, beta):
    ...
    for node in nodes:
        if not graph.has_edge(*(root, node)):
            ...
            score.append(((root, node), powerterm * n))
...
def evaluate(predicted_edges, graph):
    ...
    print ('******************')
    print (predicted_set & alledges)
    print('******************')
    return len(predicted_set & alledges) / len(predicted_set)"""
    ...
    return intersection / len(predicted_edges)
    pass
